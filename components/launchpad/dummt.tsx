...
export const LaunchpadDetails = ({ id }: { id: string | undefined }) => {
  const launchpadINfO = launchpadOptions(id ? id : '');
  const { data } = useSuspenseQuery(launchpadINfO)
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!data) return
      if (data.token_name == 'moonmunchbtc' || 'memegoatstx') return;
          ...
          >
            ...
            <div className="p-4 bg-custom-black/60 md:w-[90%] md:mx-auto">
              <div className="flex justify-between items-center md:-mt-[5rem] -mt-[3.5rem]">
                ...
                >
                  {live ? presaleClosed ? "Active" : "Closed" : data.token_name == 'memegoatstx' ? "Closed" : "Not Started"}
                </Tag>
              </div>
                      ...
                      <div className="bg-primary-60/5 backdrop-blur-[5px] text-sm w-full">
                        <div className="p-4 text-silver border-b-[1px] border-primary-20/20 mb-5">
                          <p>
                            ...
                            {live ? presaleClosed ? "JOIN PRESALE" : "PRESALE ENDED" : data.token_name == 'memegoatstx' ? "CLOSED" : "UPCOMING PRESALE"}
                          </p>
                        </div>
